...
# Default fallback path; overridden by data_dir argument passed to load_dataset()
...
class LibrispeechASR(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        # archive_path = {}
        # archive_path["train.100"] = tracked_str(path + 'train-clean-100.tar.gz')
        # archive_path['dev']  = tracked_str(path + 'dev-clean.tar.gz')
        ...
        archive_path = {}
        # local_extracted_archive['dev'] = tracked_str(local_data_path +'LibriSpeech/dev-clean/')
        # local_extracted_archive["train.100"] = tracked_str(local_data_path+'LibriSpeech/train-clean-100/')
        # archive_path["train.100"] = tracked_str(local_data_path + 'train-clean-100.tar.gz')
        # archive_path['dev']  = tracked_str(local_data_path + 'dev-clean.tar.gz')

        local_extracted_archive['dev'] = local_data_path +'LibriSpeech/dev-clean/'
        local_extracted_archive["train.100"] = local_data_path+'LibriSpeech/train-clean-100/'
        archive_path["train.100"] = local_data_path + 'train-clean-100.tar.gz'
        archive_path['dev']  = local_data_path + 'dev-clean.tar.gz'
            ...
            if path.endswith(".flac"):
                ...
                for line in f:
                    if line:
                        ...
                        audio_file = f"{speaker_id}/{chapter_id}/{id_}.flac"
                        audio_file = (
