...
def _kmeans_greedy_init(data: torch.Tensor, k: int) -> torch.Tensor:
    ...
    running_min_distances = torch.full((data.shape[0],), float('inf'), device=data.device, dtype=data.dtype)
...
def fit_kmeans(
    ...
    if greedy_init:
        ...
        clusters = data[torch.randperm(data.shape[0], dtype=torch.long)[:k], :]  # [k, dim]
    ...
    for i in range(max_iter):
        ...
        if len(devices) == 1:
            ...
            # ]
            cluster_sums = torch.zeros_like(clusters[0]).index_add_(0, nearest_indices[0], data[0])
            cluster_counts = torch.bincount(nearest_indices[0], minlength=k).unsqueeze(1).clamp_min(1)
            new_clusters = [cluster_sums / cluster_counts]
        else:
            cluster_sums = [
