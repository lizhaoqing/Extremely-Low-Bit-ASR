...
class AQEngine(nn.Module):
    ...
    def __init__(self, layer: nn.Linear, accumulator_dtype: torch.dtype = torch.float64):
        ...
        self.register_buffer(
            "X_max", torch.zeros((self.columns), dtype=accumulator_dtype, device=self.device)
        )
    ...
    def add_batch(self, inp: torch.Tensor):
        ...
        self.X_max *= self.nsamples / (self.nsamples + tmp)
        self.nsamples += tmp
        inp_sum = inp.t().to(self.X_max.dtype).abs().sum(dim=0) / self.nsamples
        inp = math.sqrt(1 / self.nsamples) * inp.to(self.XTX.dtype)
        self.XTX += inp.matmul(inp.t())
        self.X_max += inp_sum

    @torch.enable_grad()
    def joint_quantize(self, *, args: Namespace, verbose: bool = True) -> QuantizedWeight:
        ...
        print([name for name, param in self.quantized_weight.named_parameters() if (param.requires_grad or "codes" in name)])

        differentiable_parameters = nn.ParameterDict(
            {name: param for name, param in self.quantized_weight.named_parameters() if (param.requires_grad or "codes" in name)}
        )
        ...
        for epoch in range(args.max_epochs):
            ...
            for step in range(args.steps_per_epoch):
                if len(args.devices) == 1:
                    losses = []
                    losses.append(self._compute_mse(precision=-1))
                    for prec in range(args.num_precisions-1):
                        losses.append(self._compute_mse(precision=prec) * 0.5)
                    loss = sum(losses)
                else:
                    losses = []
                    losses.append(self._compute_mse_parallel(args.devices, replicas, differentiable_parameters, precision=-1))
                    for prec in range(args.num_precisions-1):
                        losses.append(self._compute_mse_parallel(args.devices, replicas, differentiable_parameters, precision=prec) * 0.5)
                    loss = sum(losses)

                if not torch.isfinite(loss).item():
                    raise ValueError(f"Quantization loss is {loss}")
                if step == 0 and args.relative_mse_tolerance is not None:
                    if loss.item() / previous_best_loss > (1.0 - args.relative_mse_tolerance) and loss.item() / previous_best_loss < 1.02:
                        print(f"loss_ratio: {loss.item() / previous_best_loss}")
    ...
    def _compute_mse(self, selection: Union[slice, ellipsis] = ..., precision = -1) -> torch.Tensor:
        """
        ...
        quantized_weight = self.quantized_weight(selection, precision)
    ...
    def _replace_and_compute_mse(self, params_to_replace: nn.ParameterDict, selection: slice, precision) -> torch.Tensor:
        ...
        return self._compute_mse(selection, precision)

    def _compute_mse_parallel(
        self, devices: Sequence[torch.device], replicas: Sequence[AQEngine], parameters_to_replicate: nn.ParameterDict, precision=-1
    ) -> torch.Tensor:
        ...
        inputs_by_replica = [(dict(), active_slices_by_replica[0], precision)]
        for i in range(1, len(devices)):
            inputs_by_replica.append((replicated_parameters[i], active_slices_by_replica[i], precision))
    ...
    def _replace_and_beam_search(self, params_to_replace: nn.ParameterDict, selection: slice, **kwargs) -> torch.Tensor:
        ...
        dtype = self.quantized_weight.codebooks0.dtype
        for param_name, param_value in params_to_replace.items():
            replace_parameter_(self.quantized_weight, param_name, param_value)
        out_channel_selection = slice(
            selection.start * self.quantized_weight.out_group_size,
        ...
        **kwargs,
        ...
        if len(devices) == 1:  # single device
            ...
            self.quantized_weight.beam_search_update_codes_(
                self.XTX.to(dtype), self.layer.weight.detach().to(dtype), dim_rng=random.Random(seed), precision=precision, **kwargs
            )
        else:
            ...
            kwargs_by_replica = [dict(kwargs, dim_rng=random.Random(seed), precision=precision) for _ in range(len(devices))]
            ...
            for device, replica in zip(devices, replicas):
                codes = replica.quantized_weight.get_codes(precision)
