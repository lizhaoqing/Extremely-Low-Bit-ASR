...
def main():
    ...
    if args.backend == "pytorch":
        ...
        filter = []
        states = torch.load(last[-1], map_location=torch.device("cpu"))["model"]
        for k in states.keys():
            if 'linear.weight' in k or 'conv.weight' in k or 'linear.bias' in k or 'conv.bias' in k:
                filter.append(k)

        # last = [last[len(last)-kk-1] for kk in range(len(last))]
