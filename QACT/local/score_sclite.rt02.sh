...
rt03* )
 ...
 # Score only the, swbd part 1...
  if [ ${stage} -le 3 ]; then
        swbd1_stm=${score_dir}/ref.swbd1.stm 
        swbd1_ctm=${score_dir}/hyp.swbd1.ctm
        ${cmd} ${score_dir}/score.swbd1.log \
          grep -v '^sw_' ${stm} '>' ${swbd1_stm} '&&' \
          grep -v '^sw_' ${ctm} '>' ${swbd1_ctm} '&&' \
          ${hubscr} -p ${hubdir} -V -l english -h hub5 -g ${data}/glm -r ${swbd1_stm} ${swbd1_ctm} || exit 1;
  fi
 # Score only the, swbd part 2...
  if [ ${stage} -le 3 ]; then
        swbd2_stm=${score_dir}/ref.swbd2.stm 
        swbd2_ctm=${score_dir}/hyp.swbd2.ctm
        ${cmd} ${score_dir}/score.swbd2.log \
          grep -v -E '^sw_4\|^sw4' ${stm} '>' ${swbd2_stm} '&&' \
          grep -v -E '^sw_4\|^sw4' ${ctm} '>' ${swbd2_ctm} '&&' \
          ${hubscr} -p ${hubdir} -V -l english -h hub5 -g ${data}/glm -r ${swbd2_stm} ${swbd2_ctm} || exit 1;
  fi
 # Score only the, swbd part 3...
  if [ ${stage} -le 3 ]; then
        swbd3_stm=${score_dir}/ref.swbd3.stm 
        swbd3_ctm=${score_dir}/hyp.swbd3.ctm
        ${cmd} ${score_dir}/score.swbd3.log \
          grep -v -E '^sw_3\|^sw4' ${stm} '>' ${swbd3_stm} '&&' \
          grep -v -E '^sw_3\|^sw4' ${ctm} '>' ${swbd3_ctm} '&&' \
          ${hubscr} -p ${hubdir} -V -l english -h hub5 -g ${data}/glm -r ${swbd3_stm} ${swbd3_ctm} || exit 1;
  fi
 ;;
esac
